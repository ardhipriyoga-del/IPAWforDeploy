...
import { applyRules, BillingRowContext, RuleMatchResult } from '../lib/billingRuleEngine';
...
function parseBillingFile(
  ...
  for (const r of raw) {
    ...
    if (rawDate instanceof Date) {
      date = rawDate.toLocaleDateString('id-ID', { day: '2-digit', month: '2-digit', year: 'numeric' });
    } else if (rawDate) {
      date = String(rawDate);
    }
  ...
  const processFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      const result: BillingRow[] = parsed.map(row => {
        ...
        const ruleResult = applyRules(activeRules, ctx) ?? undefined;
        return ruleResult ? { ...row, ruleResult } : row;
      });
      setRows(result);
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-muted/60 border-b border-border">
                    ...
                    <th className="text-right px-3 py-2.5 font-semibold text-muted-foreground whitespace-nowrap">Tarif Master</th>
                ...
                <tbody className="divide-y divide-border">
                  {paginated.length === 0 ? (
                    ...
                    paginated.map((r, i) => (
                      <tr key={i} className={`${rowBg(r.status)} hover:brightness-95 transition-all`}>
