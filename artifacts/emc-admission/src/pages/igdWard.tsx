...
import { RefreshCw, Search, X, Clock, User, MapPin, Activity, Stethoscope, AlertCircle } from 'lucide-react';
...
function getPatientWard(p: IGDPatient): WardKey {
  ...
  // No doctor or unrecognised location → Nurse Station
...
async function fetchIGDWardData(): Promise<IGDPatient[]> {
  // Via API proxy (Replit / configured server)
  if (hasApiProxy()) {
  ...
  // Direct browser fetch (hospital internal network)
...
// Client-side parser fallback (same logic as server, no timerTransfer filter)
...
function getTimerBadgeColor(timerColor: string): string {
  ...
  if (c.includes('merah') || c.includes('red')) return 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400';
  if (c.includes('kuning') || c.includes('yellow')) return 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400';
  if (c.includes('hijau') || c.includes('green')) return 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400';
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-1.5 mb-1.5">
        ...
        <span className={`text-[11px] leading-tight ${hasDokter ? 'text-gray-700 dark:text-gray-300' : 'text-amber-600 dark:text-amber-400 italic'}`}>
      ...
      <div className="flex gap-1 flex-wrap">
        ...
        {hasTransfer && (
          <span className={`text-[10px] px-1.5 py-0.5 rounded font-mono ${getTimerBadgeColor(patient.timerColor)}`}>
...
const WardBox: React.FC<WardBoxProps> = ({
  title, wardKey, patients, colorClass, headerBg, onCardClick, minHeight = 'min-h-[160px]'
}) => (
  ...
  return (
    <Dialog open={!!patient} onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle className="text-base leading-tight">{patient.nama || 'Detail Pasien'}</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-3 py-1">
          ...
          {(patient.timerOutpatient && patient.timerOutpatient !== '--') && (
            <div>
              ...
              <p className="text-sm font-mono text-blue-600 dark:text-blue-400">{patient.timerOutpatient}</p>
            </div>
          )}
          ...
          {(patient.timerTransfer && patient.timerTransfer !== '--') && (
            <div>
              ...
              <p className={`text-sm font-mono font-semibold ${getTimerBadgeColor(patient.timerColor)} inline-block px-2 py-0.5 rounded`}>
    ...
    <div className="min-h-full bg-gray-50 dark:bg-gray-950 p-4">
      ...
      {!loading || patients.length > 0 ? (
        <div className="space-y-4">
          ...
          {/* ── Row 1: Nurse Station (full width) ─────────────────────────── */}
          <WardBox
            title="Nurse Station"
          ...
          {/* ── Row 2: Procedure Room (Ruang Tindakan) ─────────────────────── */}
          <WardBox
