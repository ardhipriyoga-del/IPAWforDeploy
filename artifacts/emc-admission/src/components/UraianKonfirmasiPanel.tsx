...
type FormData = Omit<UraianKonfirmasi, 'noRM' | 'riwayat' | 'updatedAt' | 'updatedBy'>;
...
export default function UraianKonfirmasiPanel({ patient, user, onClose }: Props) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const data = await db.get('uraianKonfirmasi', patient.noRM);
      if (data) {
        const { riwayat: r, noRM: _n, updatedAt: _t, updatedBy, ...rest } = data;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await db.put('uraianKonfirmasi', record);
                            ...
                            <tr key={row.id} className="hover:bg-muted/10 align-top">
                              ...
                              <td className="px-2 py-2 border border-border align-top">
                                {isEditMode ? (
                                  ...
                                  <span className="text-xs leading-relaxed">{row.tanggal}</span>
                                )}
                              </td>
